...
(function(){
	...
	var Version = '0.6.2-gobblegobble';
	var lastUpdate = '2009.11.22';
    ...
    var deleteMessages = function(url) {
        ...
        if ($("#timeline").length == 0 || $("li.direct_message").length == 0) {
            if ($('#delete_dm_sent').attr("checked") && (url.indexOf("/sent") < 0)) {
                ...
                $.getJSON(url, '', function(data){
                   $("#timeline").replaceWith(data["#timeline"]);
                   deleteMessages(url);
                 });
                return;
            } else {
        ...
        // }
        var tweet_list = $("#timeline .direct_message");
